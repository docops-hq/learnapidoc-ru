...
Чтобы извлечь элемент скорости ветра из ответа JSON и распечатать его в консоли JavaScript, добавляем его в пример кода (который вы создали в [предыдущем разделе](inspect-json.md)) прямо под строкой `console.log (response)`:
