...
Подробная информация для этого практического занятия находится по ссылкам [Проверка JSON из ответов нагрузки](../like-developer/inspect-json.md) и [Доступ и печать определенного значения JSON](../like-developer/access-and-print-value.md)
...
### Практическое занятие "Оценка безадресного контента на 3 сайта API документации""
